        ...
        if 'accounts' in st.session_state and st.session_state.accounts:
            ...
            with col2:
                if st.button("Copy Suggested Password"):
                    if 'suggested_password' in st.session_state:
                        password_manager.copy_to_clipboard(st.session_state.suggested_password)
                        st.success("Password copied to clipboard! It will be cleared after 10 seconds.")
            ...
            if st.button("Generate Passwords"):
                ...
                for account, password in passwords_dict.items():
                    ...
                    if st.button(f"Copy password for {account}"):
                        password_manager.copy_to_clipboard(password)
                        st.success(f"Password for {account} copied to clipboard! It will be cleared after 10 seconds.")
